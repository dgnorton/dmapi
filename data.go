...
func (e *Entries) Find(startDate, endDate, pattern, workoutType string, workoutFelt string) (*Entries, error) {
	...
	for _, entry := range e.Entries {
		...
		if useFelt && entry.Workout.Felt != workoutFelt {
			continue
		}
		if startDate != "" {
			t, err := entry.Time()
